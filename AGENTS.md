# pdf-viewer — Point d'entrée agent (Codex + Antigravity)

> **Projet multi-agent.** Ce fichier est lu par **Codex** et **Antigravity**.
> Gemini Code Assist lit `GEMINI.md` (miroir maintenu en sync).
> La source de vérité pour tous les agents est le wiki `memory/`.
...
Ce projet utilise un wiki markdown dans `memory/`.

**Commencer chaque session par :**
1. `memory/VISION.md` — philosophie et limites du projet (2 min) ← **ancrage obligatoire**
2. `memory/INDEX.md` — catalogue de toutes les pages
3. `memory/LOG.md` — 10 dernières entrées (ce qui a changé)
4. `memory/fixes-registry.md` — comportements critiques à ne JAMAIS régresser
5. `memory/PRD.md` — si tu travailles sur une nouvelle feature (priorités, hors-scope)

**Avant de modifier `backend/main.py` ou `backend/pipeline.py` :**
Lire `memory/fixes-registry.md` entièrement. Grep le snippet "Code clé" de chaque FIX
avant de toucher la fonction concernée. Snippet absent → STOP, alerter l'utilisateur.

**Après tout changement de code :**
Suivre le PROTOCOLE ÉCRITURE en bas de ce fichier.
...
## PROTOCOLE DÉBOGAGE (quand un bug est signalé)
1. **Lire `memory/LOG.md`** — les 10 dernières entrées : le bug vient souvent d'une régression introduite par une modification récente.
2. **Lire `memory/fixes-registry.md` en entier** — chercher si un FIX existant couvre la zone touchée. Un nouveau symptôme dans une zone déjà fixée signale une régression, pas un bug nouveau.
3. Grep le "Code clé" du FIX suspect dans le code source pour vérifier qu'il est bien présent.
4. Si le snippet est absent → le fix a été écrasé → restaurer avant toute autre action.
5. Seulement si aucun FIX existant n'explique le bug → investiguer comme un bug nouveau.

> **Règle :** Ne jamais diagnostiquer un bug sans avoir d'abord croisé `LOG.md` + `fixes-registry.md`. La cause se trouve souvent dans une interaction entre deux fixes récents (ex. : FIX-047 parallélisme + FIX-048 thread-safety pypdfium2).

---

## PROTOCOLE LECTURE (nouveau agent / nouvelle session)
...
2. Si nouvelle contrainte introduite → ajouter FIX-NNN dans `memory/fixes-registry.md`
   (incrémenter N à partir du dernier existant)
...
7. **Mettre à jour le tableau FIX dans AGENTS.md ET dans GEMINI.md** si de nouveaux FIX sont ajoutés
...
|-------|------------------------|-------|
| Codex | `AGENTS.md` (ce fichier) | Agent principal |
| Antigravity | `AGENTS.md` (ce fichier) | Lit le même fichier |
| Gemini Code Assist | `GEMINI.md` | Miroir de ce fichier |

**Règle de synchronisation :** Quand le tableau FIX ou les protocoles changent ici,
mettre à jour `GEMINI.md` en même temps. Source de vérité détaillée : `memory/fixes-registry.md`.
