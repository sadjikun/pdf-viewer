...
cd /d "%~dp0"
set "ROOT=%~dp0"
title pdf-viewer - Lanceur IA

echo.
echo  +--------------------------------------+
echo  ^|      pdf-viewer  -- start AI         ^|
echo  ^|   Florence-2  +  Texify  actifs      ^|
echo  +--------------------------------------+
echo.
...
if not exist "backend\.venv\Scripts\activate.bat" (
    ...
    echo       Lance install.bat d'abord, puis relance start-ai.bat.
    echo.
    pause & exit /b 1
)
...
(
    echo @echo off
    echo title  Backend IA :!BACKEND_PORT!
    echo cd /d "!ROOT!backend"
    echo call .venv\Scripts\activate
    echo set FLORENCE2_CAPTION=1
    echo set FORMULA_ENGINE=texify
    echo set HF_ENDPOINT=https://hf-mirror.com
    echo echo.
    echo echo  =^> Backend IA : http://127.0.0.1:!BACKEND_PORT!
    echo echo  =^> Florence-2  : ACTIVE  ^(captioning figures^)
    echo echo  =^> Texify       : ACTIVE  ^(OCR formules^)
    echo echo.
    echo uvicorn main:app --reload --reload-exclude .venv --port !BACKEND_PORT!
    echo pause
) > "%TEMP%\pv_backend_ai.cmd"

(
    echo @echo off
    echo title  Frontend :!FRONTEND_PORT!
    ...
    echo npm run dev
    echo pause
) > "%TEMP%\pv_frontend_ai.cmd"

REM ── Lancer Windows Terminal : 1 fenetre, 2 volets cote a cote ─────────────
echo  [1/2] Ouverture de Windows Terminal...
wt new-tab --title "Backend IA" cmd /k "%TEMP%\pv_backend_ai.cmd" ; split-pane -V --title "Frontend" cmd /k "%TEMP%\pv_frontend_ai.cmd"
...
echo.
echo  [OK] pdf-viewer IA lance !
echo       Backend  --^> http://127.0.0.1:!BACKEND_PORT!
echo       Frontend --^> http://localhost:!FRONTEND_PORT!
echo.
echo  Modules IA actifs :
echo    - Florence-2  : descriptions automatiques des figures
echo    - Texify      : OCR LaTeX des formules mathematiques
echo.
echo  Note : le premier traitement d'un PDF tele-charge les modeles IA.
echo         Florence-2 ~450 MB   Texify ~500 MB
echo.
