...
| TD-025 | 2026-05-30 | **`MarkdownReader.tsx` (2289 l.) toujours monolithique** : les 5 hooks extraits dans `hooks/` (PR #20) + `buildExportHtml.ts` + test `sectionizeHtml.test.ts` sont prets, mais **non cables** dans le composant. Le cablage (remplacement des useState inline par les imports hooks) reste a faire avec navigateur ouvert. | FAIBLE | EN COURS | Cabler les 5 hooks dans MarkdownReader.tsx avec un navigateur pour valider chaque etape. |
